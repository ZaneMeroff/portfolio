    ...
    <section>
      ...
      <div className={ gameBoardVisible ? 'gameboard-outer-container' : 'hidden' }>
        ...
        <div className={ disableButtons ? 'top-section disabled' : 'top-section' }>
          <button tabIndex='0' aria-label='red button' onClick={() => handlePlayerResponse(0)} className={ redActive ? 'red-box red-active' : 'red-box' }></button>
          <button tabIndex='0' aria-label='yellow button' onClick={() => handlePlayerResponse(1)} className={ yellowActive ? 'yellow-box yellow-active' : 'yellow-box' }></button>
        </div>
        <div className={ disableButtons ? 'bottom-section disabled' : 'bottom-section' }>
          <button tabIndex='0' aria-label='green button' onClick={() => handlePlayerResponse(2)} className={ greenActive ? 'green-box green-active' : 'green-box' }></button>
          <button tabIndex='0' aria-label='blue button' onClick={() => handlePlayerResponse(3)} className={ blueActive ? 'blue-box blue-active' : 'blue-box' }></button>
        </div>
      </div>
    </section>
  );
